...
| `StateError` | Đỏ | `(3, 0, 0)` | **Lỗi** — Lỗi hệ thống (reserved) | Lỗi nghiêm trọng | Có — khi lỗi được khắc phục |
| `StateOTA` | Xanh lá | `(0, 3, 0)` | **Đang update** — OTA firmware đang chạy (enum dự trữ; bootstrap drive LED OTA trực tiếp qua `lib/hal` — xem "Bootstrap (OTA)" bên dưới) | Bootstrap reconcile phát hiện update | Khởi động lại sau khi update xong |
| `StateBooting` | Xanh dương | `(0, 1, 3)` | **Đang khởi động** — Lamp đang bật | `server.go` lúc startup | Có — khi OpenClaw agent connect và sẵn sàng |
| `StateLeLampDown` | Tím | `(2, 0, 3)` | **HAL Down** — Server phần cứng không phản hồi. Khi HAL đang down LED **tắt hẳn** vì driver LED cũng chết theo; tím breathing chỉ flash ~3s khi phục hồi | `healthwatch` poll HAL `/health` thất bại | Tự tắt 3s sau khi phục hồi |
| `StateAgentDown` | Cyan | `(0, 3, 3)` | **Agent Down** — AI brain mất kết nối | OpenClaw WebSocket ngắt (`runtimes/openclaw/service_ws.go`) | Có — khi WebSocket reconnect |
...
|---|---|---|
| Đang tải + cài | Cam `(3, 1, 0)` `breathing` speed 0.4 | `bootstrap/bootstrap.go` |
| Thành công | Xanh lá `(0, 3, 1)` `notification_flash` ngắn rồi dừng | `bootstrap/bootstrap.go` |
| Thất bại | Đỏ `(3, 1, 1)` `pulse` speed 1.5 | `bootstrap/bootstrap.go` |
