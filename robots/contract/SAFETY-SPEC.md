...
| `audio.max_volume` | no | **enforced (v1)** | Integer `0–100` (%). `POST /audio/volume` clamps any higher request, above the speaker/Bluetooth-sink split, so the ceiling binds every caller alike — agent, local intent, web slider, boot restore. All-day and independent of `audio.quiet_hours`. Reported on `GET /audio/volume` as `max_volume` so a UI can bound its own control instead of snapping back. (Slice 5.) |
