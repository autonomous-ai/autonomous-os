...
#
# What it does:
#   1. prerequisites: jq/curl/git/python3 + the python websockets library the
#      bridge needs;
...
#      it OWNS the bridge (bridge.py), the launch env (/root/.claudecode/.env —
#      ANTHROPIC_* from config.json llm_*), and the telegram channel config
...
#      so the unit runs the bridge (python3 bridge.py), which holds the headless
#      Claude process and exposes the WebSocket os-server connects to.
#
...
#
# ⚠️ VERIFY ON DEVICE: the bridge must listen on 127.0.0.1:18791 and serve the
#    WebSocket at /claude/ws/ to match internal/claudecode/constants.go WSURL,
#    and its bearer token must equal constants.go Token (both are baked into
#    bridge.py by presync.sh).
set -euo pipefail
...
echo "[install-claudecode] prerequisites (jq, curl, git, python3, websockets)"
apt-get update || true
apt-get install -y jq curl git python3 || true
# The bridge needs the python websockets library. Prefer the distro package;
# fall back to pip (bookworm needs --break-system-packages).
...
python3 -c 'import websockets' || {
  echo "[install-claudecode] ERROR: python websockets library unavailable" >&2
  exit 1
}
...
# Bridge + env + channel config are owned ENTIRELY by the presync hook, NOT
# written here. The hook is materialized to /usr/local/bin/runtime-claudecode-presync
# by os-server BEFORE this installer runs; switch-runtime re-runs it before every
# later start, and EnsureOnboarding re-runs it on every os-server boot — so the
# config self-heals (e.g. after a factory reset).
PRESYNC_HOOK="/usr/local/bin/runtime-claudecode-presync"
if [ -x "$PRESYNC_HOOK" ]; then
  echo "[install-claudecode] materialize bridge + sync env/channels now (via $PRESYNC_HOOK)"
  "$PRESYNC_HOOK" \
    || echo "[install-claudecode] WARN: presync failed (config.json missing? non-fatal — retried on next switch/boot)"
else
  echo "[install-claudecode] WARN: $PRESYNC_HOOK absent — os-server did not materialize it (standalone/offline run?); bridge NOT configured"
fi

# systemd unit — the bridge wraps the foreground-only claude process so
# switch-runtime can enable/disable/verify it like any other backend. Unit name
# == runtime name (claudecode.service), so no service-name declaration file is
# needed. KEEP IN SYNC with internal/claudecode/gateway_unit.go.
...
# Verify hook: cheap + offline (CLI presence only — per the adding-agent-runtime
# golden rule, presync heals everything else, so a structure check here would
# only force needless full reinstalls).
echo "[install-claudecode] declare verify hook for switch-runtime (command -v claude)"
...
echo "[install-claudecode] done — claudecode bridge installed + started (claudecode.service)."
