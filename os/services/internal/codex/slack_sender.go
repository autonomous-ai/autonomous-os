...
// Slack Web API client for the Codex runtime, mirroring hermes'
// slack_sender.go minus the streaming methods (chat.startStream /
// appendStream / stopStream / assistant status) — codex posts the reply once.
...
func (s *CodexService) slackAPI(method string, payload map[string]any, benignErrors ...string) (slackAPIResponse, error) {
...
// chat.postMessage.
func (s *CodexService) postSlackMessage(channel, threadTS, text string) error {
...
// success.
func (s *CodexService) setSlackReaction(add bool, channel, ts, name string) error {
	if channel == "" || ts == "" {
		return nil
	}
