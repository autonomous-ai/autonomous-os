...
- **`software-update`** — the on-device OTA updater installed at `/usr/local/bin/software-update`, and the **single source of truth** for it. Every fleet gets this one file: the imagers `install -m 0755` it from `/input` (staged by `imager/Makefile` before `docker run`, because the Docker build context is `imager/` and cannot reach this directory), `release/upload-setup.sh` inlines it into `setup.sh` between the `>>> BEGIN software-update (generated)` markers at publish time, and `release/upload-device.sh` stages it at the root of every device-profile package (boards that install from the package — Reachy's `spike-bootstrap.sh` — read it from `/opt/devices/<type>/software-update`). `upload-setup.sh` additionally publishes it raw at `{CDN}/software-update`, the only way to heal a device already in the field (an old updater silently skips components it does not know — see the `updaterSupports` gate in `docs/bootstrap-ota.md` §4): `sudo curl -fsSL https://cdn.autonomous.ai/os/software-update -o /tmp/su && sudo bash -n /tmp/su && sudo install -m 0755 /tmp/su /usr/local/bin/software-update`. The `setup.sh` copy in git is a placeholder that exits 1 — assembly happens at release, never in git, so it cannot go stale. Edit it here and nowhere else.
