...
EOF
  ...
  for skill_name in $SKILLS_CATALOG; do
    ...
    if [ "$skill_name" = environment ] && ! echo "$DEVICE_CAPS" | grep -qx environment; then
      echo "[stage] skill environment requires explicit environment capability — skipping"
      continue
    fi
