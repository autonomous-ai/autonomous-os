...
- **Route requests with Jev.** For normal voice requests received by os-server and text-only Web/MQTT chat, local rules run first, then the Jev intent fallback (enabled by default). The OS validates the selected command, parameters, confidence, and device capabilities before execution; uncertain requests continue to the main agent. Context-dependent follow-ups bypass intent handling when they need the main agent. Attachments and Harness-only voice retain their separate routes. Jev also powers [Hermes skill preloading](docs/agentic/hermes.md#13-optional-jev-skill-preloading) and experimental [Buddy UI-action suggestions](docs/os-server.md#buddy-computer-use-feedback); these are separate integrations.
