...
Harness-only voice history uses the same `[HANDLED]` / `[REPLY]`, `NO_REPLY` and `MarkSilentRun` convention as realtime, via the source-neutral `system/externalhistory` journal. This does not move or alter realtime's existing HAL notification path or silent/TTS handling. See [external conversation history](os-server.md#external-conversation-history).
