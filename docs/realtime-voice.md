...
Gemini extended-thinking camera replay: when the existing look replay marker is armed, an interruption of the deferred filler retires its grace, outcome checks and buffered continuation. The replay receives a fresh response generation while preserving the user question; an old terminal on the interruption frame cannot finish the visual answer. Ordinary interruptions and LIVE mode retain their cancellation behavior. The new response still needs a confirmed outcome; this does not force visual requests to succeed or disable fallback. Diagnostic: `look_replay_response_started`.
