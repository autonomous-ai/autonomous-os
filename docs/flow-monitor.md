...
External history runs with a valid `device-chat-context-` ID and attributed `[external-context]` / `[HANDLED]` / `[REPLY]` envelope display as **History sync**, with **Harness → Main** (or the external source). Cards show the original question and reported answer as **Context**, not a new TTS response. Agent name is available on the route tooltip; raw metadata remains in event details. Parsing uses complete chat-send data when chat-input previews are truncated, and does not change lifecycle status.
