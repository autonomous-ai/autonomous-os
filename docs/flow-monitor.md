...
- `look.capture` → **monitor-only** `sensing_input` event, emitted by HAL when the realtime `look` tool captures a frame (`hal/realtime/look_monitor.py`). The handler logs the flow event and returns — it is **never forwarded to the agent**, because the frame already went straight to the realtime model and a forwarded message would inject a phantom turn. The thumbnail is derived from the frame path in the message by the existing `/api/sensing/agent-snapshot/:runtime/:source/:name` route, so no copy is made. Unlike a `motion.activity` snapshot (surfaced in the UI but stripped before the LLM), **this frame IS what the model saw** — making it the artefact to compare against the model's answer.
