...
| `HAL_REALTIME_AI_REJECT_FILTER` | `true` | Đăng ký `reject_turn` và bật policy gate tách riêng `should_drop_realtime_rejection()`. Chỉ tool call rõ ràng mới bỏ transcript trước OS dispatch; model im lặng, timeout hay lỗi vẫn fallback sang main agent. Đặt `false` để tắt filter thử nghiệm này mà không đổi phần routing realtime còn lại. |
