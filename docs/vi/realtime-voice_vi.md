...
History Harness-only voice dùng cùng quy ước `[HANDLED]` / `[REPLY]`, `NO_REPLY` và `MarkSilentRun` như realtime, qua journal chung `system/externalhistory`. Thay đổi này không di chuyển hay sửa đường notification HAL realtime hoặc silent/TTS hiện có. Xem [lịch sử hội thoại từ bên ngoài](os-server_vi.md#lịch-sử-hội-thoại-từ-bên-ngoài).
