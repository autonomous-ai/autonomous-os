...
Replay camera với Gemini extended-thinking: khi cơ chế đánh dấu replay sau look đã được bật, interruption của filler đang chờ sẽ kết thúc grace, kiểm tra outcome và continuation buffer của phản hồi cũ. Replay nhận generation phản hồi mới và giữ câu hỏi của người dùng; terminal cũ trên cùng frame interruption không được kết thúc câu trả lời về ảnh. Interruption thông thường và LIVE mode giữ nguyên hành vi cancel. Phản hồi mới vẫn cần outcome được xác nhận; thay đổi này không ép yêu cầu ảnh thành công hoặc tắt fallback. Log chẩn đoán: `look_replay_response_started`.
