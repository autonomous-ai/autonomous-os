...
Tiêu đề tab trình duyệt (`document.title`) hiển thị đúng theo page/tab đang focus, để mở nhiều tab Lamp vẫn phân biệt được. Dùng hook chung `useDocumentTitle` (`os/services/web/src/hooks/useDocumentTitle.ts`); format: `Lamp · <segment>[· <sub-segment>]`.
...
- **HAL** (blue): từ `GET /api/system/info` → field `lelampVersion`. Lamp tự gọi `:5001/version` của HAL qua loopback mỗi phút 1 lần (cache) rồi re-expose qua API của lamp, browser không cần truy cập trực tiếp `/hw/*` (nginx chặn `/hw/` chỉ cho loopback).
...
**Turn Pipeline (SVG)** — `FlowDiagram` trong `os/services/web/src/pages/Monitor.tsx`. Bố cục đầy đủ (ba vùng Lamp / HAL / OpenClaw, lưới cột OpenClaw, Cron thuộc Lamp, hàng HAL thẳng Tool, bảng tọa độ) nằm trong **`docs/flow-monitor.md`**; tóm tắt tiếng Việt: **`docs/vi/flow-monitor_vi.md`**.
