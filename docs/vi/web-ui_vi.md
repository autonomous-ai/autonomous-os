...
- Timeout 120 giây tính theo **idle**: mỗi SSE event thuộc run đang chờ (`assistant_delta`, `thinking`, tool call) đều đẩy lùi hạn, nên một turn chạy nhiều phút vẫn ở trạng thái pending chừng nào agent còn làm việc; chỉ 120 giây im lặng thật sự mới bỏ cuộc. Khi bỏ cuộc: giữ phần text đã stream làm câu trả lời, không có thì báo lỗi kèm nút retry. Cố ý KHÔNG phải hạn tuyệt đối — hạn tuyệt đối từng chốt một turn build dài thành "no response" trong khi run vẫn đang chạy; MQTT chat và Telegram không dính vì chúng không có deadline phía client.
