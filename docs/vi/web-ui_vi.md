...
- **Khôi phục turn đang chờ sau reload**: message lưu kèm epoch `ts`; bubble reply đang pending dưới 10 phút sẽ sống sót qua reload thay vì bị chốt thành lỗi. Ở lần render đầu khi tab Chat active, UI re-attach vào `runId` đã lưu và backfill câu trả lời từ flow JSONL replay (`/api/agent/flow-stream` gửi lại 500 event cuối trong ngày mỗi lần connect — `tts_send` / `tts_suppressed` / `no_reply` / `harness_response`). Recovery dùng cùng ngân sách idle 12 phút với turn đang chạy: reload khi Harness hoặc Codex vẫn làm việc không được chốt lỗi trước khi terminal event được ghi. Event live làm mới deadline này.
