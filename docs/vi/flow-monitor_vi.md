...
### `<think>` — chỗ để phần suy nghĩ

Trên fleet này model không có kênh thinking riêng: cả một ngày turn, os-server nhận **0** event `stream: "thinking"` trong khi assistant text vẫn chảy bình thường (đo trên `lamp-a0ae` ngày 2026-09-10, openclaw + `Auto-AI`). Nên model nghĩ ngay trong chỗ duy nhất nó có — text trả lời — và thiết bị đọc hết. Một lượt ghi được: câu trả lời chỉ 2 câu nhưng **2036 ký tự** ra loa, mở đầu bằng *"This is a fresh `[voice-instruction]` — the realtime agent delegated it to me… Per the connector skill I must read the payload back…"*.

`<think>…</think>` cho phần suy nghĩ đó một chỗ để đi. Nó là cặp đối của `<say>`: **`<say>` chọn phần DUY NHẤT được nói, `<think>` đánh dấu phần KHÔNG được nói.** Hai cái độc lập — một reply có thể dùng cái này, cái kia, cả hai, hoặc không cái nào.

- **Bị cắt** bởi `stripThinkTag` (`handler_text.go`), gọi ngay trước `extractSayTag` ở cả 4 đường reply: `lifecycle:end`, error recovery, channel-turn finalize, và đường `session.message`. Nên khối này không tới TTS lẫn web chat / channel fan-out. Raw delta trong `agent_last_token` vẫn giữ nguyên để debug.
- **Hoãn stream câu đầu** khi khối còn đang mở (`hasOpenThinkTag`, `handler_state.go`) — soi theo guard `<say>` có sẵn. Câu đã ra loa thì không rút lại được, nên không stream gì cho tới khi `</think>` tới; sau đó câu đầu của **câu trả lời** vẫn stream sớm, giữ nguyên lợi thế first-audio. Cùng guard đó chặn luôn Slack stream giữa turn, vì diff append-only cũng không rút text lại được.
- **Offset khớp nhau**: `tryFirstSentenceFlush` cắt các khối đã đóng trước khi đo `streamedCleanLen`, nên offset trỏ vào đúng chuỗi mà `lifecycle:end` sẽ cắt.
- **Fail open.** `<think>` không đóng, hoặc reply chỉ có mỗi khối think — cả hai đi qua nguyên vẹn và vẫn được nói. Máy im còn tệ hơn máy nói dài, mà nói dài thì vốn đã là hành vi hiện tại. Cả hai trường hợp đều log `WARN`.

Model được dạy về khối này trong khối prompt do OS quản lý mà mỗi runtime inject lúc boot (`agentsMDBlock` / `claudeMDBlock` / `soulSkillPriorityBlock`), nằm trong quy tắc *Say the answer, not the reasoning*.
