...
compile vào binary.) Installer kéo Hermes CLI về `/usr/local/bin/hermes`, chạy `hermes claw migrate`
(chỉ skills), seed `~/.hermes/{.env,config.yaml}`, tạo `hermes.service` ở trạng
thái **chưa enable**, và drop hook `runtime-hermes-presync` (§11).

`hermes claw migrate` **không** mang model config qua, nên hook presync sync
`llm_*` của thiết bị từ `config.json` vào config Hermes mỗi lần switch:
...
1. đảm bảo `X.service` tồn tại, nếu chưa thì chạy installer của `X` — ưu tiên bản
   embed ở `/usr/local/lib/os-runtimes/X/install.sh`, không có thì
