...
Đường thực thi: người dùng/voice → `harness-use` → API loopback OS → kết nối trực tiếp đã xác thực → Harness CLI → agent được chọn trên máy tính. Skill chờ receipt rồi đọc recap của agent; sau đó device agent tạo một câu trả lời cuối duy nhất. Voice đọc chính câu đó, còn Web Chat hiển thị chính câu đó. Callback Harness không được đưa thành JSON sensing event và cũng không đọc raw `turn.summary`, nên không tạo turn thứ hai hoặc TTS khác nội dung chat. Khi người dùng yêu cầu một agent làm việc, kể cả research bằng browser, `harness-use` được ưu tiên; `computer-use` dành cho thao tác UI Mac trực tiếp và Buddy chỉ dùng khi người dùng gọi rõ.
