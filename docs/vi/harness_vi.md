...
Với đèn có MPR121, vuốt **phải sang trái** rồi nhả để bật/tắt Harness voice; **trái sang phải** gọi sleep. `swipe_axis` liệt kê điện cực theo thứ tự trái sang phải vật lý (Lamp mặc định E0…E11; cần kiểm tra chiều lắp). Action worker hiện có của Python gọi API Go; Go quyết định mode và focus. Khi bật, giữ focus hợp lệ hiện tại; nếu chưa có, `focus.ensure` chọn agent đầu tiên trong registry cục bộ của CLI đã pair và chờ Desktop xác nhận đã chọn. Không có agent, máy offline, app không khả dụng hoặc chưa hỗ trợ chọn focus ban đầu thì mode vẫn tắt. Không thay thế focus đang trỏ sang máy khác. Tắt mode vẫn hoạt động khi offline. Thiết bị chưa ghép đôi trả `harness_unpaired`; HAL nói “Bạn cần ghép đôi thiết bị trong ứng dụng Harness trước.” Đã ghép đôi nhưng mất kết nối trả `harness_offline`. Cả hai lỗi đều giữ mode tắt. HAL đọc kết quả thực tế bằng phrase theo ngôn ngữ cấu hình: Anh, Việt, Trung giản thể hoặc Trung phồn thể, kèm LED báo ngắn; thành công nói rõ đã bật Harness cùng tên agent, hoặc đã tắt Harness và trở về trợ lý trên thiết bị. Gesture này không áp dụng cho GPIO/TTP223. Web/MQTT vẫn giữ hành vi set trực tiếp khi offline/chưa focus. Chỉ tự chọn ban đầu khi bật bằng gesture, không đổi đích của câu nói.

Luồng là microphone → kiểm tra VAD, noise và echo hiện có → STT text cuối → controller Harness trong OS → agent CLI đang focus. Harness ON bỏ qua wake gate cho đầu vào và tín hiệu đang nghe, không kéo dài timer wake window chung. OFF khôi phục gate cũ ở capture tiếp theo; sleep và mute mic vẫn được ưu tiên. HAL đọc mode chính thức trước capture và bỏ qua Realtime khi bật; OS dispatch trước local intent và các gate readiness/busy của runtime chính. Mỗi capture mang generation định tuyến. Thay đổi mode hoặc target làm capture cũ hết hiệu lực, không chuyển hay phát lại text sang agent khác. Đổi mode giữa một live capture sẽ ngắt capture đó, không replay. Đổi focus trong lúc capture khiến capture bị từ chối và người dùng cần nói lại; CLI kiểm tra revision focus để từ chối send hoặc answer đã cũ. Web/MQTT chat dạng text và ambient sensing giữ đường xử lý hiện có.
...
| `POST /api/harness/voice-mode/gesture` | Chỉ loopback thực sự | Bật/tắt bằng `{gestureId:"<UUID>"}`. Thành công trả snapshot mode; lỗi action trả `status:0` và `data.code` để HAL đọc phrase bản địa hóa. Cache RAM giữ 128 kết quả gesture thành công/thất bại gần nhất để ID lặp không bật/tắt hai lần; lệnh off rõ ràng từ web/MQTT hủy gesture đang chờ bật. |
