...
- **Seed-khi-rỗng:** lúc boot `device.ProvideService` gọi
  ...
  `gateway.default` trong ROBOT.md về sau KHÔNG còn ảnh hưởng device đã seed — phải
...
| **HAL passthrough** | `SendToHALTTS*`, `StopTTS`, `SetVolume`, `StartHALVoice` | Thường giống nhau giữa các backend — share hoặc copy. `StartHALVoice(cfg hal.VoiceStartConfig) error` nhận một struct (không phải tham số theo vị trí) — xem `system/lib/hal/client.go` để biết ý nghĩa từng field (`STTProvider`/`STTModel`/`TTSModel` chọn provider+model STT/TTS; `STTProvider` rỗng = auto-selection legacy của HAL). Cả sáu backend hiện có implement giống hệt nhau: no-op trừ khi `cfg.DeepgramKey != ""`, ngược lại gọi `hal.StartVoice(cfg)`. |
