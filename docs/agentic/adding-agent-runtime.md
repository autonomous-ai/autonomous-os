...
- **Seed-on-empty:** at boot `device.ProvideService` calls
  ...
  becomes a no-op. Corollary: changing `gateway.default` in ROBOT.md afterwards no
...
| **HAL passthrough** | `SendToHALTTS*`, `StopTTS`, `SetVolume`, `StartHALVoice` | Usually identical across backends — share or copy. `StartHALVoice(cfg hal.VoiceStartConfig) error` takes one struct (not positional args) — see `system/lib/hal/client.go` for field semantics (`STTProvider`/`STTModel`/`TTSModel` select the STT/TTS provider+model; empty `STTProvider` = HAL's legacy auto-selection). All six existing backends implement it identically: no-op unless `cfg.DeepgramKey != ""`, else `hal.StartVoice(cfg)`. |
