...
- **Jev:** the OS-managed Hermes plugin selects and preloads an installed skill before the first model call, with normal skill discovery as fallback. This plugin is enabled in the current build; the separate voice-intent fallback is off by default, while Buddy's action-suggestion endpoint is enabled but experimental. Selection does not execute actions or grant permission. See [Hermes preloading](agentic/hermes.md#13-optional-jev-skill-preloading), [intent fallback](os-server.md#jev-intent-fallback), and [Buddy suggestions](os-server.md#buddy-computer-use-feedback).
