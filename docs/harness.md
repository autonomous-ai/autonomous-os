...
The execution path is: user/voice → `harness-use` → OS loopback API → authenticated direct connection → Harness CLI → selected computer agent. A completed `turn.summary` returns its final `text` (or `recap` when text is empty) directly to device TTS. It is never sent back through the device agent as a JSON sensing event, so it cannot create a second unrelated agent turn or hide behind a “Sent” Web Chat reply. `harness-use` takes precedence when the user asks an agent to work, including browser research; `computer-use` is for direct visible Mac UI work and Buddy remains available only when explicitly requested.
