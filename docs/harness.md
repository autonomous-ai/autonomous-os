...
The execution path is: user/voice → `harness-use` → OS loopback API → authenticated direct connection → Harness CLI → selected computer agent. The skill waits for the receipt and reads the agent recap, then the device agent produces one canonical final response. Voice speaks that response; Web Chat displays that same response. Harness callback frames are not injected as JSON sensing events and do not speak raw `turn.summary` text, so they cannot create a second turn or a different TTS answer. `harness-use` takes precedence when the user asks an agent to work, including browser research; `computer-use` is for direct visible Mac UI work and Buddy remains available only when explicitly requested.
