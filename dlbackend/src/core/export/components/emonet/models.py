    ...
    def __init__(
        ...
        if self.temporal_smoothing:
            ...
            self.temporal_weights = (
                torch.Tensor([0.1, 0.1, 0.15, 0.25, 0.4])
                .unsqueeze(0)
                .unsqueeze(2)
                .cuda()
            )  # Size (1,5,1)
        ...
        if self.attention:
            n_in_features = 256 * (
                num_modules + 1
            )  # Heatmap is applied hence no need to have it
        else:
        ...
        self.emo_convs = []
        self.conv1x1_input_emo_2 = nn.Conv2d(
            n_in_features, 256, kernel_size=1, stride=1, padding=0
        )
    ...
    def forward(self, x, reset_smoothing=False):
        ...
        if self.init_smoothing:
            ...
            self.temporal_state = torch.zeros(
                x.size(0), self.n_temporal_states, self.n_expression + self.n_reg
            ).cuda()
        if reset_smoothing:
