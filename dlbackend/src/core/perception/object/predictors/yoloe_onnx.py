"""YOLO-E zero-shot object detector using ONNX Runtime.

The ONNX graph includes the MobileCLIP text encoder. NMS is run in
postprocess when nms=True. Output boxes are normalized [0, 1] relative
to the letterboxed image — use ``unletterbox_boxes()`` for pixel coords.
"""
...
class YOLOEONNXDetector(ONNXObjectDetector):
    """Zero-shot object detection using YOLO-E ONNX model."""

    DEFAULT_MODEL_PATH: Path | None = None
    DEFAULT_THRESHOLD: float = 0.25
    DEFAULT_IMGSZ: int = 640
    DEFAULT_TEXT_MODEL: str = "mobileclip:blt"
    ...
    def _start_tokenizer(self) -> None:
        self._logger.info("Loading MobileCLIP text encoder: %s", self._text_model_name)
    ...
    def _build_onnx_inputs(
        ...
        if self._clip_model is None:
            raise RuntimeError("MobileCLIP text encoder not loaded")
