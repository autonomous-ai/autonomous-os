// Package tracking is the device-side pipe for product analytics events.
//
// It is deliberately generic: voice KPI is the first tracker, more will
...
func Report(ev Event) {
	...
	if global.isDuplicate(ev.ID) {
		...
		slog.Info(logPrefix+" duplicate event ignored", "component", "tracking",
	...
	default:
		...
		slog.Warn(logPrefix+" event dropped -- queue full", "component", "tracking",
...
func (r *reporter) send(ctx context.Context, ev Event) {
	...
	params["tracking_dropped_total"] = r.dropped.Load()
	params["tracking_failed_total"] = r.failed.Load()
	...
	if err := r.sender(sendCtx, ev.Name, params); err != nil {
		...
		// tell you.
		slog.Warn(logPrefix+" delivery failed", "component", "tracking",
			"event_name", ev.Name, "event_id", ev.ID, "error", err, "failed_total", n)
		return
	}
	slog.Debug(logPrefix+" delivered", "component", "tracking",
		"event_name", ev.Name, "event_id", ev.ID)
}
