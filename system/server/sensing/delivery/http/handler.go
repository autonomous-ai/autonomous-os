...
const harnessFollowupRouting = "[system-routing: A Harness task or question may be awaiting a follow-up. An explicit new target, a new Harness task (including digital work delegated by the Lamp persona without naming Harness), or a request for Buddy takes priority over this follow-up hint. For a new task, read harness-use and select a suitable existing agent under the device persona policy instead of inheriting the retained target. Use the retained Harness target only when the user's words clearly continue the retained Harness task, answer its currently open question, or ask whether it has finished or for its result. Do not treat vague fragments, acknowledgements, filler, unrelated new requests, or uncertain speech as continuations; let the main agent handle those under its normal persona and skill policy. For such a continuation, use the retained target and send only the user's current words. Only when the user's words could continue a task of a different Harness agent, compare them with the listed agents' recap headlines: if exactly one listed agent's recap describes that task, continue with that agent instead; if several do, ask which task; a missing recap is unknown, not a reason to switch. Do not use Buddy and do not answer a clear Harness follow-up yourself. For a clear continuation with the skill already loaded, do not reread the skill/directory or run harness.py --help. If send/answer returns queued, delivered, started, completed, or rejected, make no more Harness or shell calls (including receipt, status, recap, list, or another send) and immediately reply NO_REPLY. Inspect receipt only for DeliveryUnknown/no usable receipt or an explicit user delivery-state request; never resend automatically.]"
