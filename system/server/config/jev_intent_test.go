...
func TestJevIntentDefaultOffAndConfigRoundTrip(t *testing.T) {
	c := Config{}
	if got := c.JevIntentSettings(); got.Enabled || got.TimeoutMS != 350 {
		t.Fatalf("unset config should be off: %+v", got)
	}
	if err := json.Unmarshal([]byte(`{"jev_intent":{"enabled":true,"timeout_ms":200}}`), &c); err != nil {
		t.Fatal(err)
	}
...
func TestJevIntentSettings(t *testing.T) {
	for _, tc := range []struct {
		...
		{"default budget", true, true, 0, 350},
		{"negative budget", true, true, -1, 350},
		{"master switch", true, false, 500, 500},
		{"bounded", true, true, 999999, 1000},
	} {
...
func TestJevUsesConfiguredProxyOnly(t *testing.T) {
	...
	if got.Endpoint != "https://proxy.example.test/api/v1/ai/v1/jev/decisions" || got.APIKey != c.LLMAPIKey || got.Enabled {
		t.Fatal("proxy routing/auth or default-off changed")
	}
	c.JevIntent = &JevIntentConfig{TimeoutMS: 200}
	if c.JevIntentSettings().Enabled {
		t.Fatal("omitted enabled must remain off")
	}
	c.LLMBaseURL = ""
