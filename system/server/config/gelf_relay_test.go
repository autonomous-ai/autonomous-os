...
func TestGELFRelayCredentials(t *testing.T) {
	const shipped = "https://campaign-api.autonomous.ai/api/v1/ai/v1"
	const staging = "https://campaign-api.staging.autonomousdev.xyz/api/v1/ai/v1"
	cases := []struct {
		name     string
		cfg      *Config
		wantBase string
		...
		{
			name:     "unversioned campaign-api base is normalized",
			cfg:      &Config{LLMBaseURL: "https://campaign-api.autonomous.ai/api/v1/ai", LLMAPIKey: "k"},
