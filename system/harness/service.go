...
var capabilities = []string{"focus.get", "agents.list", "turn.send", "turn.stop", "status", "recap", "question.answer", "receipt.get"}
...
func (s *Service) Request(ctx context.Context, frame Frame) (Frame, error) {
	...
	if kind != "focus.get" && kind != "agents.list" && kind != "receipt.get" {
