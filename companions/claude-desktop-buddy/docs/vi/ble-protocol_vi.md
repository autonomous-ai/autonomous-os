...
Connect handler của BlueZ bắn cho **mọi** thiết bị Bluetooth chạm vào adapter
dùng chung — kể cả thiết bị không liên quan (ví dụ một tai nghe được OS pair
trên cùng radio) — nên sự kiện connect thô ở tầng transport **không** điều
khiển state của buddy. Thiết bị chỉ tuyên bố "Claude Desktop đã kết nối" khi có
dữ liệu đến trên characteristic NUS RX (chỉ client Desktop mới ghi vào đó), và
gán phiên đó cho transport connect gần nhất. "Ngắt kết nối" chỉ bắn khi đúng
client đã gán rớt (hoặc link cuối cùng rớt). Sự kiện connect/disconnect của các
thiết bị khác chỉ được log dạng `transport connected/disconnected` và bị bỏ qua.
