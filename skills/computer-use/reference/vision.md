...
Use an actual display ID from `list_displays`; 1 above is a placeholder example. Choose the user's target display, normally `is_main:true`. Pick a capture scale suited to the model and text size; a width around 1280–1600 pixels is a reasonable first observation, then increase resolution when text is unreadable. Compute scale from the chosen display's `pixel_width`, cap at 1, and use the **returned actual image dimensions** for coordinate conversion. There is no universally required 1280-pixel width.
