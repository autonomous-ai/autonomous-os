...
describe('workspace and Git boundaries', () => {
  it('reads actual status, history, staged and unstaged diff, and untracked files', async () => {
    ...
    expect(git.files).toContainEqual({ path: 'file.txt', status: 'MM', lineStats: { added: 1, removed: 1 } })
    expect(git.files).toContainEqual({ path: 'new.txt', status: '??', lineStats: { added: 1, removed: 0 } })
...
describe('committed branch review', () => {
  it('uses the merge base, excludes index and working edits, and pins previews to snapshot commits', async () => {
    const { projectPath, manager, project } = await setup()
    await gitCommand(projectPath, ['branch', '-M', 'main'])
    const baseHash = (await gitCommand(projectPath, ['rev-parse', 'HEAD'])).trim()
    await gitCommand(projectPath, ['checkout', '-b', 'feature'])
    await writeFile(path.join(projectPath, 'feature.txt'), 'committed branch line\n')
    await gitCommand(projectPath, ['add', 'feature.txt'])
    await gitCommand(projectPath, ['commit', '-m', 'Feature'])
    const headHash = (await gitCommand(projectPath, ['rev-parse', 'HEAD'])).trim()
    await gitCommand(projectPath, ['checkout', 'main'])
    await writeFile(path.join(projectPath, 'base-only.txt'), 'later main change\n')
    await gitCommand(projectPath, ['add', 'base-only.txt'])
    await gitCommand(projectPath, ['commit', '-m', 'Main diverges'])
    await gitCommand(projectPath, ['checkout', 'feature'])
    await writeFile(path.join(projectPath, 'feature.txt'), 'index edit\n')
    await gitCommand(projectPath, ['add', 'feature.txt'])
    await writeFile(path.join(projectPath, 'feature.txt'), 'working edit\n')
    await writeFile(path.join(projectPath, 'untracked.txt'), 'untracked\n')
    const branch = (await manager.git(project.id, projectPath)).branchChanges!
    expect(branch).toEqual({ base: 'main', baseHash, headHash, files: [{ path: 'feature.txt', status: 'A', lineStats: { added: 1, removed: 0 } }] })
    const patch = await manager.branchDiff(project.id, projectPath, branch.baseHash, branch.headHash, 'feature.txt')
    expect(patch).toContain('+committed branch line')
    expect(patch).not.toMatch(/index edit|working edit|later main change/)
    await gitCommand(projectPath, ['commit', '-m', 'Move feature tip'])
    expect(await manager.branchDiff(project.id, projectPath, branch.baseHash, branch.headHash, 'feature.txt')).toBe(patch)
    await expect(manager.branchDiff(project.id, projectPath, '--stat', headHash, 'feature.txt')).rejects.toThrow('full commit hash')
    await expect(manager.branchDiff(project.id, projectPath, baseHash, headHash, 'untracked.txt')).rejects.toThrow('not part')
    await expect(manager.branchDiff(project.id, tmpdir(), baseHash, headHash, 'feature.txt')).rejects.toThrow()
  })

  it('prefers verified origin HEAD and preserves renamed unusual paths and unknown binary stats', async () => {
    const { projectPath, manager, project } = await setup()
    await gitCommand(projectPath, ['branch', '-M', 'trunk'])
    await gitCommand(projectPath, ['update-ref', 'refs/remotes/origin/trunk', 'HEAD'])
    await gitCommand(projectPath, ['symbolic-ref', 'refs/remotes/origin/HEAD', 'refs/remotes/origin/trunk'])
    await gitCommand(projectPath, ['checkout', '-b', 'feature'])
    const name = 'renamed\tfile\n.txt'
    await gitCommand(projectPath, ['mv', 'file.txt', name])
    await writeFile(path.join(projectPath, 'binary.dat'), Buffer.from([0, 1, 2, 3]))
    await gitCommand(projectPath, ['add', 'binary.dat'])
    await gitCommand(projectPath, ['commit', '-m', 'Rename and binary'])
    const branch = (await manager.git(project.id, projectPath)).branchChanges!
    expect(branch.base).toBe('origin/trunk')
    expect(branch.files).toContainEqual({ path: name, originalPath: 'file.txt', status: 'R100', lineStats: { added: 0, removed: 0 } })
    expect(branch.files.find((file) => file.path === 'binary.dat')?.lineStats).toBeUndefined()
    expect(await manager.branchDiff(project.id, projectPath, branch.baseHash, branch.headHash, name)).toContain('similarity index 100%')
  })

  it('does not invent a base from the feature upstream or the current local default branch', async () => {
    const { projectPath, manager, project } = await setup()
    await gitCommand(projectPath, ['branch', '-M', 'main'])
    expect((await manager.git(project.id, projectPath)).branchChanges).toBeUndefined()
    await gitCommand(projectPath, ['branch', '-M', 'feature'])
    await gitCommand(projectPath, ['update-ref', 'refs/remotes/origin/feature', 'HEAD'])
    await gitCommand(projectPath, ['config', 'branch.feature.remote', 'origin'])
    await gitCommand(projectPath, ['config', 'branch.feature.merge', 'refs/heads/feature'])
    expect((await manager.git(project.id, projectPath)).branchChanges).toBeUndefined()
  })
})
