...
Panel phải tách `CHANGES` cho file đã theo dõi và `UNTRACKED FILES` cho trạng thái Git porcelain `??`, mỗi nhóm có nút thu gọn và số lượng riêng; ẩn nhóm rỗng. Stage/unstage vẫn tác động đường dẫn file rõ ràng. `COMMITTED ON BRANCH` mặc định mở bên dưới các nhóm, có đường phân cách. Khi working tree sạch, một dòng thông báo gọn thay hình minh họa lớn để danh sách file đã commit hiện gần đầu panel. Số dòng của file đã theo dõi vẫn là diff gộp HEAD đến working tree, không phải diff riêng index.
