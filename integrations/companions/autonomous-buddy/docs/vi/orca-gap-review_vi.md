...
| Tab và sở hữu nội dung | [Tab model][layout] có group đệ quy, group/worktree/entity ID, preview khác tab cố định, nhiều content type. | **Một phần.** `renderer/App.tsx` hỗ trợ rename và đóng session bằng ×/Cmd+W: dừng/lưu trữ session, giữ history, bỏ khỏi sidebar qua restart. Đóng cửa sổ macOS giữ các session khác chạy. Preview file/diff tạm; layout pane được lưu, chưa reorder/pin tab hoặc chuyển nội dung giữa group. | Core: open/close/reopen/active tab và preview nhất quán trước đổi giao diện. |
| Terminal pane song song | [Terminal layout][terminal] lưu leaf ID ổn định, focus/expand, buffer từng leaf; [split actions][split] split phải/dưới kế thừa cwd thực tế. | **Một phần.** `renderer/SessionWorkspace.tsx` và `split-layout.ts` có split đệ quy lưu bền, Terminal N thật, focus/input riêng và resize. Đóng pane dừng/lưu trữ session đích, giữ session khác; snapshot mới loại leaf đã lưu trữ. Split dùng root worktree. Chưa đặt agent có sẵn, chuyển pane giữa tab hay phím chuyển focus. | Core: kiểm chứng input isolation/restore, sau đó đặt agent có sẵn và cwd thực tế. |
| Start/follow-up/stop/sleep/resume | [Sleeping record][resume] gắn provider ID, transcript path, launch env và nguồn restore; [hibernation][hibernate] không kill trước khi lưu record resume được. | **Một phần.** `main/manager.ts`, `main/providers.ts` mặc định chạy CLI Codex/Claude interactive thật. Stop/Sleep giữ session; Resume CLI dùng đúng ID provider đã biết, từ chối ID chưa biết. Voice create trì hoãn khởi chạy tới prompt argv đầu. Session structured cũ giữ history và resume theo lượt. Restart terminal chạy shell mới, không phục hồi tiến trình sống hay tự wake. | Core: nói rõ sleep/wake/restore; giữ hội thoại trước khi tuyên bố resume. |
| Agent state và input tương tác | [Status][status] phân biệt working/blocked/waiting/done, không suy từ terminal title; [approval card][approval] gửi lựa chọn về đúng PTY; [chat subscriptions][chat] gắn frame transcript vào subscription/session. | **Một phần.** Xterm/PTY thật và hook Claude/Codex riêng session cung cấp ID, ready/working/needs-input/completed/error; `processActive` theo dõi tiến trình riêng. Follow-up vào cùng PTY chỉ khi hook xác minh readiness và không có input tay dang dở. Giữ event structured cũ. CLI xử lý câu hỏi; chưa có approval card riêng hay đầy đủ tool/message model. Cờ full access theo policy no-approval đã chọn. | Core: waiting/error/completion và follow-up đúng context. UI approval tùy chọn với policy hiện tại, không phải lý do bật lại prompt. |
| Review và hoàn tất thay đổi | [Bulk actions][stage] stage/unstage path đã chọn; [commit area][commit] có action commit/stage/push; [combined diff][diff] tách branch/commit/working và mở/edit file. | **Một phần.** `renderer/GitPanel.tsx` và `main/git.ts` có branch/commit gọn, stage/unstage từng file hoặc toàn bộ path đang hiển thị, số dòng thêm/xóa thật khi khả dụng và commit toàn bộ index, gồm file staged từ trước. Bỏ thống kê chưa biết/binary. Preview chỉ đọc; chưa combined/branch diff, hunk action, edit/save hay push. | Core: xác minh tác động index chính xác; combined review tiếp theo. Push vẫn là scope riêng tường minh. |
...
| Settings, keyboard, navigation | [Key registry][keys] định nghĩa file/worktree/settings navigation; [pane shortcut][panekeys] có split. [Workflow settings][settings] dùng metadata tìm kiếm chung cho Git/terminal/các mục khác. | **Core Appearance đã triển khai; navigation/customization rộng hơn vẫn một phần.** [Cài đặt desktop](./settings_vi.md) có mục Settings / ⌘, trong menu macOS, trang đầy đủ có tìm kiếm, theme System/Dark/Light, zoom/font UI, font/giãn dòng/cursor terminal và bật tắt footer, lưu qua settings main process. Mục Agents và Computer & device có điều khiển hoạt động thật. `renderer/App.tsx` giữ search/new-session/close; chưa command registry/palette, remap shortcut, quick-open file, MRU tab, focus-next-pane. | Core: phím dễ khám phá cho workspace/tab/pane; settings rộng để sau. |
