...
**Trạng thái hiện tại:** UI Agent Manager tạm bị vô hiệu hóa bằng cờ cố định trong mã nguồn, không có tùy chọn bật lại lúc chạy. App không tạo hoặc mở workspace khi khởi động, kích hoạt từ Dock, mở instance thứ hai, nhấn notification hay nhận action native. Mục **Open Agent Manager** trên menu bar và các menu Electron **Settings**, **Edit**, **View**, **Window** được ẩn. Backend agent bridge, pairing, Activity và quản lý quyền native vẫn hoạt động. Các mô tả UI bên dưới ghi lại chức năng được giữ trong mã nguồn để dùng khi bật lại.

**Cập nhật app:** Menu native và Electron có Check for Updates mà không mở Agent Manager. App macOS đóng gói kiểm tra và tải trong nền; khởi động lại để cài dùng luồng dừng session/helper như khi thoát bình thường. Xem [cập nhật app](app-updates_vi.md).
...
Chuyển sang completed, needs-input hoặc error đánh dấu session chưa đọc; session chỉ đánh dấu đã đọc khi pane đó và cửa sổ đều focus. Khi UI được bật lại, nhấn desktop notification mở/focus đúng session; hiện thao tác này không mở workspace. App gửi desktop notification khi chuyển sang các trạng thái này lúc cửa sổ không focus, tùy hỗ trợ/quyền của OS. Notice hoàn tất/cần chú ý cũng đi qua WebSocket Swift đã pair về device. Reconnect phát lại trạng thái kết thúc chưa đọc; delivery là best effort và notice có thể lặp.
