...
Gợi ý được hardcode **OFF** bằng `Enabled` trong `system/buddy/jev`; không thêm block config. Khi tắt, request trả gợi ý null mà không quan sát Mac hay gọi proxy. Khi deployment chủ động bật, handler tự lấy `get_ui_tree` native mới với thời hạn 5000 ms. Không nhận cây hoặc screenshot do caller gửi. Quan sát mới làm mất hiệu lực reference cũ, kể cả khi inference sau đó fallback.
