...
Suggestions are hardcoded **OFF** by `Enabled` in `system/buddy/jev`; there is no additional config block. Disabled calls return a null suggestion without observing the Mac or calling the proxy. When explicitly enabled in a deployment, the handler requests a fresh native `get_ui_tree` with a 5000 ms deadline. Caller-supplied trees and screenshots are not accepted. A fresh observation invalidates earlier references, including when inference later falls back.
