...
Local macOS packaging: from `desktop/`, `make build` compiles, rebuilds node-pty, packages the current architecture and verifies an ad-hoc signature. `make install` copies the verified bundle to `/Applications/Autonomous Buddy.app` with staged replacement, preserving the separate Swift companion. `make open` opens it. This is not a notarized distribution build. Packaged startup supplements inherited PATH with the login shell PATH (5-second timeout) and common CLI directories so Finder launches can discover installed agents. Set `BUDDY_APP_EXECUTABLE` to the packaged executable to run the same Electron smoke suite against the bundle.
