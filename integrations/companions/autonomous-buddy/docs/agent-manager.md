...
**Current availability:** The Agent Manager UI is temporarily disabled by a fixed source-code flag with no runtime override. The app does not create or open the workspace on startup, Dock activation, a second instance, notification clicks, or native actions. The menu-bar **Open Agent Manager** item and Electron **Settings**, **Edit**, **View**, and **Window** menus are hidden. The backend agent bridge, pairing, Activity, and native permission controls remain available. UI descriptions below document functionality retained in source for when the workspace is re-enabled.

**App updates:** The native and Electron menus expose Check for Updates without opening Agent Manager. Packaged macOS apps check and download in the background; restarting to install uses the normal session/helper shutdown path. See [app updates](app-updates.md).

Autonomous Buddy is one desktop application with two internal components. Electron's main process owns agent processes, Git/file access, persistence and the React workspace implementation. The embedded `macos/` Swift helper owns pairing, the device WebSocket and native computer-use executors. Users install and open one `Autonomous Buddy.app`; the helper is a child process, not a second application to install.
...
Completed, needs-input and error transitions mark a session unread. It is marked read only while its pane and document are focused. When the UI is enabled, clicking a desktop notification opens/focuses that session; while disabled, the click does not open the workspace. Desktop notifications are requested on those transitions while the window is unfocused, subject to OS support/permissions. Completion/attention notices also travel through the paired Swift WebSocket to the device. Reconnect republishes unread terminal states; delivery is best effort and notices may repeat.
