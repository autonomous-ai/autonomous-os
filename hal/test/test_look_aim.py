...
def test_the_sweep_says_something_at_its_midpoint():
    """One phrase at the start does not cover half a minute of swinging.

    Repeating `look_searching` instead would ask "where are you?" twice, which
    sounds stuck rather than patient — hence its own pool.
    """
    from hal.drivers.tracking import search

    said = []

    def fake_search(target="person", detector=None, on_progress=None):
        for visited in range(1, 10):
            on_progress(visited, 9)
        return search.SearchResult(False, "nobody found", 9)

    with mock.patch.object(search, "search_for_subject", fake_search), \
         mock.patch.object(aim, "_say", side_effect=said.append):
        _REAL_SWEEP()
...
def test_a_sweep_that_ends_early_says_nothing_extra():
    ...
    def fake_search(target="person", detector=None, on_progress=None):
        on_progress(1, 9)
        on_progress(2, 9)
        return search.SearchResult(True, "found person", 2, 40.0)

    with mock.patch.object(search, "search_for_subject", fake_search), \
         mock.patch.object(aim, "_say", side_effect=said.append):
        assert _REAL_SWEEP() is True
