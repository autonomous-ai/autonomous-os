...
relayed through campaign-api (`{base}/logs/gelf`) with the device's lobster key,
...
CAMPAIGN = "https://campaign-api.autonomous.ai/api/v1/ai/v1"
...
def test_gelf_url_env_ships_direct_with_basic_auth():
    ...
    url, auth, headers = resolve_target(env, _cfg({"llm_base_url": CAMPAIGN, "llm_api_key": "k"}))
    assert url == "https://logs.example/gelf"
    assert auth == ("u", "p")
    assert headers == {}


def test_unset_gelf_url_relays_through_campaign_api_with_bearer():
    url, auth, headers = resolve_target({}, _cfg({"llm_base_url": CAMPAIGN, "llm_api_key": "lobster"}))
    assert url == CAMPAIGN + "/logs/gelf"
    assert auth is None
...
def test_relay_prefers_shipped_autonomous_defaults_over_byo_llm_fields():
    cfg = _cfg({
        "autonomous_defaults": {"base_url": CAMPAIGN, "api_key": "shipped"},
...
def test_relay_partial_defaults_fall_back_to_llm_fields():
    cfg = _cfg({
        "autonomous_defaults": {"base_url": CAMPAIGN, "api_key": ""},
        "llm_base_url": CAMPAIGN,
        "llm_api_key": "live",
    })
...
def test_relay_ignores_non_dict_defaults():
    cfg = _cfg({"autonomous_defaults": "", "llm_base_url": CAMPAIGN, "llm_api_key": "live"})
    url, _, headers = resolve_target({}, cfg)
    assert url == CAMPAIGN + "/logs/gelf"
    assert headers == {"Authorization": "Bearer live"}
...
def test_relay_normalizes_unversioned_campaign_api_base():
    cfg = _cfg({"llm_base_url": "https://campaign-api.autonomous.ai/api/v1/ai/", "llm_api_key": "k"})
...
def test_handler_relay_posts_bearer_without_basic_auth(monkeypatch):
    ...
    handler = GELFHandler(os_cfg_get=_cfg({"llm_base_url": CAMPAIGN, "llm_api_key": "lobster"}))
    handler._send({"short_message": "hi"})

    session = _FakeSession.instances[0]
    assert session.posts == [(CAMPAIGN + "/logs/gelf", {"short_message": "hi"})]
