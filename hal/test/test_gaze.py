...
def _in_conversation(monkeypatch):
    """Pretend the follow-up window is open."""
    import hal.app_state as state

    monkeypatch.setattr(
        state, "voice_service",
        type("V", (), {"_wakeword_focus": _OpenFocus()})(),
        raising=False,
    )


def test_the_framing_loops_hold_still_during_a_conversation(neck, monkeypatch):
    """Centring the face serves the wake decision, and it is already made.

    Correcting on through a conversation answers a question nobody is asking,
    in the most visible currency the lamp has: it creeps after every lean while
    the user is mid-sentence. The pose is not abandoned — idle stays anchored on
    the one that framed them.
    """
    _in_conversation(monkeypatch)
    _fill_dy(-0.4, from_face=True)
    gaze._maybe_pitch(gaze.time.monotonic())
    assert neck.moves == [], "the lamp moved to re-centre mid-conversation"


def test_a_pan_is_held_off_during_a_conversation_too(body, monkeypatch):
    _in_conversation(monkeypatch)
    monkeypatch.setattr(config, "GAZE_YAW_ENABLED", True)
    gaze._last_yaw_t = gaze.time.monotonic() - 10_000.0
    _fill_dx(0.4)
    gaze._maybe_yaw(gaze.time.monotonic())
    assert body.moves == []


def test_a_prompted_climb_still_runs_during_a_conversation(neck, monkeypatch):
    """The exemption: a climb somebody ASKED for is not unprompted creeping."""
    _in_conversation(monkeypatch)
    _fill_dy(-0.5, from_face=False)
    gaze._blind_pitch_steps = 0
    gaze._maybe_pitch(gaze.time.monotonic(), prompt=True)
    assert neck.moves, "an explicitly requested climb must not be held off"


def test_holding_still_can_be_turned_off(neck, monkeypatch):
    _in_conversation(monkeypatch)
    monkeypatch.setattr(config, "GAZE_HOLD_STILL_IN_CONVERSATION", False)
    _fill_dy(-0.4, from_face=True)
    gaze._maybe_pitch(gaze.time.monotonic())
    assert neck.moves


def test_a_pitch_correction_keeps_the_heading_it_did_not_touch(neck, monkeypatch):
    """The bug: a vertical fix quietly threw away a completed horizontal one.

    An anchor is rebuilt from scratch and handed over whole, and a joint MISSING
    from it does not mean "leave that one alone" — it eases back to the
    recording's baseline. So while the anchor listed only the pitch joints, this
    sequence undid itself: a yaw correction anchored base_yaw and wrist_roll (it
    merges its own target in), then the next pitch correction rebuilt the anchor
    without them and idle walked the lamp's heading back to the middle of the
    room, with the user still sitting where it had just turned to face them.
    """
    turned = {"base_yaw.pos": 24.0, "wrist_roll.pos": 8.0}
    monkeypatch.setattr(
        neck, "get_positions",
        lambda: {**_PitchSvc.get_positions(neck), **turned},
    )
    anchored = {}
    neck.set_idle_anchor = lambda j: anchored.update(j or {})
    gaze._last_anchor = None
    ...
    assert anchored, "a correction must anchor something"
    for joint, held in turned.items():
        assert anchored.get(joint) == pytest.approx(held), (
            f"{joint} dropped out of the anchor, so idle will pull it back to "
            f"the recording: {sorted(anchored)}"
        )


def test_the_heading_can_be_left_out_of_the_anchor(neck, monkeypatch):
    """GAZE_IDLE_ANCHOR_YAW off restores the pitch-only anchor exactly."""
    monkeypatch.setattr(config, "GAZE_IDLE_ANCHOR_YAW", False)
    monkeypatch.setattr(
        neck, "get_positions",
        lambda: {**_PitchSvc.get_positions(neck), "base_yaw.pos": 24.0},
    )
    anchored = {}
    neck.set_idle_anchor = lambda j: anchored.update(j or {})
    gaze._last_anchor = None
    _fill_dy(-0.4, from_face=True)
    gaze._maybe_pitch(gaze.time.monotonic())
    assert anchored and "base_yaw.pos" not in anchored


def test_the_remembered_bearing_anchors_the_heading_too(body, monkeypatch):
    """A repoint turns to the remembered bearing; idle must rest on it.

    The vertical half of a remembered pose is refused once the pitch loop has
    spoken (it is stale — see above), but the heading never is: that is the one
    thing the bearing memory is unambiguously good for, and it is what the move
    itself uses. Leaving yaw out made idle the single consumer that pulled the
    lamp back off a bearing every other consumer had just agreed on.
    """
    from hal.drivers.tracking import user_bearing

    class _EstFacing(_Est):
        pose = {"base_yaw.pos": 31.0, "wrist_roll.pos": -6.0,
                "base_pitch.pos": 0.0, "wrist_pitch.pos": -70.0}

    monkeypatch.setattr(user_bearing, "read_estimate", lambda: _EstFacing())
    anchored = {}
    body.set_idle_anchor = lambda j: anchored.update(j or {})
    _absent_for(config.GAZE_REPOINT_AFTER_S + 1)
    # Pitch HAS spoken, so the vertical half is refused — the heading must
    # survive that refusal rather than be dropped alongside it.
    gaze._last_pitch_t = gaze.time.monotonic() - config.GAZE_REPOINT_AFTER_S - 1.0
    gaze._last_anchor = None
    gaze._maybe_repoint(gaze.time.monotonic())
    assert anchored.get("base_yaw.pos") == pytest.approx(31.0)
    assert not (set(anchored) & set(gaze.PITCH_JOINTS))
