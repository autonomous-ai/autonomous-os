...
# campaign-api path that relays GELF records to Graylog (bff-campaign-service).
# Appended to the device's campaign-api base URL, which already ends in /v1.
...
def _normalize_base(base: str) -> str:
    """Mirror of system/lib/urlnorm.NormalizeBaseURL. os-server normalizes
    config.json only when it saves it, so a never-re-saved file can still hold
    the unversioned campaign-api base."""
    base = base.strip().rstrip("/")
    if "campaign-api.autonomous.ai" in base and base.endswith("/ai"):
        base += "/v1"
    return base
...
def _relay_credentials(os_cfg_get: Callable[..., Any]) -> tuple[str, str]:
    """Pick the campaign-api base URL + device key for the relay, or ("", "").
...
def resolve_target(
    ...
    Unset → relayed through campaign-api (`{base}/logs/gelf`) with the device's
    lobster key as a Bearer token; bff-campaign-service holds the Graylog
    credential, so the device carries none. ("", None, {}) ships nothing.
    """
