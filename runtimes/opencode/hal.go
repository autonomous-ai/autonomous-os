...
func (s *OpenCodeService) Speak(text string) error {
	...
	slog.Info("TTS spoken (verbatim)", "component", "opencode", "text", truncRunes(text, 80))
...
func (s *OpenCodeService) SendToHALTTSQueueForTurn(text, turnID string, turnSeq uint64) error {
	text = stripForTTS(text)
	if text == "" {
		return nil
	}
	...
	slog.Info("TTS queued", "component", "opencode", "turn_id", turnID, "turn_seq", turnSeq, "text", truncRunes(text, 80))
	s.monitorBus.Push(domain.MonitorEvent{Type: "tts", Summary: text})
	return nil
}
