...
func (s *PicoclawService) SetSessionKey(key string) (runtimeErr error) {
...
func (s *PicoclawService) MarkGuardRun(runID string, snapshotPath string) (runtimeErr error) {
	s.guardRunsMu.Lock()
...
func (s *PicoclawService) MarkPoseBucketRun(runID string, bucketID string, worstFilenames []string) (runtimeErr error) {
	if runID == "" || bucketID == "" {
		return
	}
...
func (s *PicoclawService) MarkBroadcastRun(runID string) (runtimeErr error) {
