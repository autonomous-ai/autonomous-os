...
**People — keep ` + "`USER.md`" + ` current (Claude Code has no heartbeat, so do this as you go).** ` + "`KNOWLEDGE.md`" + ` and ` + "`memory/*.md`" + ` are yours, but ` + "`USER.md`" + ` is the file loaded into every session — anything you learn about a PERSON has to reach it or you will not have it next session. When a turn teaches you something durable about someone, update their entry in ` + "`/root/.claudecode/workspace/USER.md`" + ` under a ` + "`## Users`" + ` heading, in the form ` + "`- **<label> (friend)**: …`" + ` — where ` + "`<label>`" + ` is their ENROLLMENT LABEL exactly as it appears in ` + "`[context: current_user=…]`" + `, lowercase. The ` + "`(friend)`" + ` part is required; without it the OS cannot tell your entry from a form field.
  ...
  - **Update and add only — never delete.** Someone you have not seen lately is simply not touched: absence is not departure. Retiring a person is the OS's job (it removes an entry once their face/voice enrollment is gone), not yours.
  - **Keep each entry under ~600 characters** — a few lines of what actually helps you help them. This file is loaded into every session, so bloat is billed on all of them; and when it overflows the cap it is cut from the END, which is where ` + "`## Users`" + ` lives. Rewrite an entry to stay short rather than appending to it.
  - **Strangers get NO entry.** ` + "`## Users`" + ` is for people the device knows by enrollment. A passing face has no label to key on and nothing durable to remember; note desk traffic in ` + "`KNOWLEDGE.md`" + ` if it is worth keeping at all.
