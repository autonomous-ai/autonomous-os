	...
	if s.config.DiscordManaged {
		if s.discordRelay != nil {
			if err := s.discordRelay.SendDiscordTyping(channelID); err != nil {
				slog.Debug("discord managed typing failed", "component", "claudecode", "error", err)
			}
		}
		return
	}
...
func (s *ClaudeCodeService) finishDiscordTurn(channelID, reply string) {
	...
	if s.config.DiscordManaged {
		if s.discordRelay == nil {
			slog.Error("discord managed reply dropped: no relay installed", "component", "claudecode", "channelID", channelID)
			return
		}
		if err := s.discordRelay.SendDiscordReply(channelID, reply); err != nil {
			slog.Error("discord managed reply send failed", "component", "claudecode", "channelID", channelID, "error", err)
			return
		}
		slog.Info("discord managed reply posted", "component", "claudecode", "channelID", channelID)
		return
	}
...
func (s *ClaudeCodeService) SetChannelRelay(relay domain.ChannelRelay) {
...
func (s *ClaudeCodeService) HandleInboundDiscord(in domain.DiscordInbound) (bool, error) {
	msg := discordInbound{
	...
	if !ok {
		slog.Debug("discord managed message skipped",
			"component", "claudecode", "authorId", in.DiscordUserID, "guildId", in.GuildID)
		return false, nil
	}
